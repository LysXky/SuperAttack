...
def check_subdomain_http(subdomain):
    ...
    for protocol in ['http', 'https']:
        try:
            response = requests.get(f"{protocol}://{subdomain}", timeout=5, verify=False)
...
def scan_common_ports(host, max_workers=50):
...
def analyze_domain(domain):
    ...
    for i, subdomain in enumerate(candidate_subdomains, 1):
        print(f"\rProgreso: {i}/{len(candidate_subdomains)} - {subdomain:<50}", end="", flush=True)
        
        if check_subdomain_dns(subdomain):
            http_exists, protocol = check_subdomain_http(subdomain)
            
            # Escanear puertos
            print(f"\n🔍 Escaneando puertos para {subdomain}...")
